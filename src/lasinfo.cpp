  ...
  void run()
  {
    ...
    while (lasreadopener.active())
    {
      ...
      if (file_out && !no_header)
      {
        ...
        fprintf(
            file_out, "  project ID GUID data 1-4:   %08X-%04X-%04X-%04X-%04X%08X\012", lasheader->project_ID_GUID_data_1,
                  ...
                  switch (lasreader->header.vlr_geo_key_entries[j].key_id)
                  {
                    ...
                    case 2062:  // GeogTOWGS84GeoKey
                      switch (lasreader->header.vlr_geo_key_entries[j].count)
                      {
                        ...
                        default:
                          fprintf(
                              file_out, "GeogTOWGS84GeoKey: look-up for type %d not implemented\012", lasreader->header.vlr_geo_key_entries[j].count);
                      }
                      break;
      ...
      if (check_points)
      {
        ...
        if (file_out && !no_min_max)
        {
          ...
          if (lasreader->point.extended_point_type)
          {
            ...
            fprintf(
                file_out, "  extended_classification    %6d %6d\012", lassummary.min.extended_classification, lassummary.max.extended_classification);
        ...
        if (!no_warnings && file_out && lassummary.has_fluff())
        {
          ...
          {
            fprintf(
                file_out, "WARNING: there is serious coordinate resolution fluff (x100) in %s%s%s\012", (lassummary.has_serious_fluff(0) ? "X" : ""),
      ...
      if (check_points)
      {
        ...
        if (file_out && !no_min_max)
        {
          ...
          if (wrong_entry)
          {
            ...
            for (i = 0; i < 32; i++)
              if (lassummary.classification[i]) fprintf(file_out, " %15lld  %s (%u)\n", lassummary.classification[i], LASpointClassification[i], i);
