...
static FILE* fopen7zipped(const char* filename, const char* mode)
{
	...
	if (mode[0] == 'r')
	{
		...
		{
			LASMessage(LAS_ERROR, "could not create pipe");
			return NULL;
		}
		...
		{
			LASMessage(LAS_ERROR, "could not set pipe output");
			return NULL;
		}
		...
		{
			LASMessage(LAS_ERROR, "could not redirect input file");
			return NULL;
		}
    ...
    // close original write end of pipe
		...
		{
			LASMessage(LAS_ERROR, "could not reconstruct stdout");
			return NULL;
		}
...
static FILE* fopenGzipped(const char* filename, const char* mode)
{
	...
	if (mode[0] == 'r')
	{
		...
		{
			LASMessage(LAS_ERROR, "could not redirect stdin");
			return NULL;
		}
		...
		{
			LASMessage(LAS_ERROR, "could not reconstruct stdin");
			return NULL;
		}
...
FILE* fopen_compressed(const char* filename, const char* mode, bool* piped)
{
  ...
  if (strcmp(filename+len-3, ".gz") == 0)
  {
    ...
    if (piped) *piped = true;
#else
    LASMessage(LAS_ERROR, "no support for gzipped input");
    return 0;
#endif
  }
  else if (strcmp(filename+len-4, ".zip") == 0)
  {
    ...
    if (piped) *piped = true;
#else
    LASMessage(LAS_ERROR, "no support for ZIPped input");
    return 0;
#endif
  }
  else if (strcmp(filename+len-3, ".7z") == 0)
  {
    ...
    if (piped) *piped = true;
#else
    LASMessage(LAS_ERROR, "no support for 7zipped input");
    return 0;
#endif
  }
  else if (strcmp(filename+len-4, ".rar") == 0)
  {
    ...
    if (piped) *piped = true;
#else
    LASMessage(LAS_ERROR, "no support for RARed input");
    return 0;
#endif
  }
  else
