...
:: specify the minimum nuber of points to be used for the percentile computation
set MINIMUM_POINTS=20

:: specify the target claification code for the points matching the percentile
set CLASSIFY_AS=8
...
lasthin -i %TEMP_DIR%\tile*.laz ^
        -step %STEP% -percentile %PERCENTILE% %MINIMUM_POINTS%  ^
        -classify_as %CLASSIFY_AS% ^
        -odix _p%PERCENTILE% -olaz ^
        -cores %NUM_CORES%

:: recreate the (less huge) thinned LAS / LAZ file

lasmerge -i %TEMP_DIR%\tile*_p%PERCENTILE%.laz ^
         -o %1 -odix _p%PERCENTILE%
